# Mini scrabble for MITx: 6.00.1x Introduction to Computer Science and Programming Using Python.
...
WORDLIST_FILENAME = "aWordGameWords.txt"
...
def isValidWord(word, hand, wordList):
    ...
    if word in wordList:
        for i in word:
            if i in hand:
                count -= 1
                trackHand[i] = trackHand.get(i, 0) - 1
                if trackHand[i] < 0:
                    return False
        if count == 0:
            return True
...
def calculateHandlen(hand):
    ...
    numOfLetters = 0
    for i in hand:
        numOfLetters = numOfLetters + hand[i]
    return numOfLetters
